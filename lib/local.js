...
_describe.whenCalledLocally = function (uri, data, cb) {
    ...
    describe('handle ' + uriStr, function () {
        beforeEach(function (cb) {
            ...
            payload = merge({}, test.data, payload);

            var rek = test.sapp.rekuest(uri).payload(payload);

            if (this.loggedInAccessToken) {
                rek.prop('accessToken', test.loggedInAccessToken);
            }

            rek.send(function (err, result) {
                test.err = err;
                test.result = result;
                cb();
            });
...
_it.shouldBeDenied = function () {
    it('should not be allowed', function () {
        ...
        var expectedStatus = this.aclErrorStatus ||
            this.sapp && this.sapp.get['aclErrorStatus'] ||
            401;
...
_it.shouldBeDeniedWhenCalledAnonymously = function (uri, data) {
    _describe.whenCalledAnonymously(uri, data, function () {
        _it.shouldBeDenied();
    });
};
...
_it.shouldBeDeniedWhenCalledUnauthenticated = function (uri, data) {
    _describe.whenCalledUnauthenticated(uri, data, function () {
        _it.shouldBeDenied();
    });
};
...
_it.shouldBeDeniedWhenCalledByUser = function (credentials, uri, data) {
    _describe.whenCalledByUser(credentials, uri, data, function () {
        _it.shouldBeDenied();
    });
};
